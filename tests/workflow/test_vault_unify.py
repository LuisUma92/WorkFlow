...
def _make_project(
    ...
    for n in notes or []:
        conn.execute(
            ...
            (n["id"], n["filename"], n["reference"], n.get("title"),
             n.get("note_type"), n.get("source_format", "md"),
             n.get("zettel_id")),
        )
    for lab in labels or []:
        conn.execute("INSERT INTO label VALUES (?, ?, ?)",
                     (lab["id"], lab["note_id"], lab["label"]))
    for ln in links or []:
        conn.execute("INSERT INTO link VALUES (?, ?, ?)",
                     (ln["id"], ln["source_id"], ln["target_id"]))
    for c in citations or []:
        conn.execute("INSERT INTO citation VALUES (?, ?, ?)",
                     (c["id"], c["note_id"], c["citationkey"]))
    for t in tags or []:
        conn.execute("INSERT INTO tag VALUES (?, ?)", (t["id"], t["name"]))
    for nt in note_tags or []:
        conn.execute("INSERT INTO note_tag VALUES (?, ?)",
...
def test_unify_empty_slipbox(tmp_path, vault_root, backup_dir, global_session):
    ...
    report = unify(project, vault_root, backup_dir=backup_dir,
                   global_session=global_session, dry_run=False)
...
def test_unify_single_project_round_trip(
    ...
    project = _make_project(
        tmp_path,
        notes=[{"id": 1, "filename": "n1.md", "reference": "ref-001",
                "title": "First", "note_type": "permanent"}],
...
def test_unify_id_collision_with_project_prefix(
    ...
    report = unify(project, vault_root, backup_dir=backup_dir,
                   ...
                   rename_strategy="project-prefix", dry_run=False)
    global_session.commit()
...
def test_unify_id_collision_abort(
    tmp_path, vault_root, backup_dir, global_session
):
    ...
    with pytest.raises(ValueError):
        unify(project, vault_root, backup_dir=backup_dir,
              global_session=global_session,
              rename_strategy="abort", dry_run=False)
    assert global_session.query(Note).count() == 1
    assert not (project / VAULT_POINTER_FILE).exists()


def test_unify_dry_run_no_writes(
    tmp_path, vault_root, backup_dir, global_session
):
    project = _make_project(
        tmp_path,
        notes=[{"id": 1, "filename": "n.md", "reference": "ref-1",
                "note_type": "permanent"}],
        md_files=[("n.md", "# n")],
    )
    report = unify(project, vault_root, backup_dir=backup_dir,
                   global_session=global_session, dry_run=True)
    global_session.commit()
...
def test_unify_idempotent(
    ...
    project = _make_project(
        tmp_path,
        notes=[{"id": 1, "filename": "n.md", "reference": "ref-x",
                "note_type": "permanent"}],
    )
    unify(project, vault_root, backup_dir=backup_dir,
          global_session=global_session, dry_run=False)
    global_session.commit()
    report2 = unify(project, vault_root, backup_dir=backup_dir,
                    global_session=global_session, dry_run=False)
    assert report2.skipped
    assert global_session.query(Note).count() == 1


def test_unify_backup_created(
    tmp_path, vault_root, backup_dir, global_session
):
    project = _make_project(
        tmp_path,
...
def test_unify_rewrites_link_fks(
    tmp_path, vault_root, backup_dir, global_session
):
    project = _make_project(
        tmp_path,
...
def test_unify_moves_md_files(
    tmp_path, vault_root, backup_dir, global_session
):
    project = _make_project(
        tmp_path,
        notes=[{"id": 1, "filename": "n1.md", "reference": "r-1",
...
def test_unify_orphan_link_detection(
    tmp_path, vault_root, backup_dir, global_session
):
    project = _make_project(
        tmp_path,
