"""Shared result types for `workflow import` (bulk hierarchy import).

Pinned contract: `engine.py` (the engine) returns an ``ImportResult``;
`formatters.py` renders it; `cli.py` maps it to exit codes. Keeping the
