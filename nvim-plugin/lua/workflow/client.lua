-- lua/workflow/client.lua
-- Forked from latexzettel/lua/latexzettel/client.lua
-- Changes: notification title updated to "workflow"
local M = {}
...
---@field config WorkflowClientConfig
...
local function notify(msg, level)
	vim.schedule(function()
		vim.notify(msg, level or vim.log.levels.INFO, { title = "workflow" })
	end)
end
...
local function default_config()
	return {
		cmd = { "python", "-m", "latexzettel.server.main" },
		protocol_version = 1,
		initialize_params = {
			client = {
				name = "workflow.nvim",
...
---@param cfg WorkflowClientConfig|nil
function M.new(cfg)
	local c = {
		job_id = nil,
		chan = nil,
...
function M.start(self)
	...
	if job_id <= 0 then
		notify("Could not start workflow server. Check PATH and dependencies.", vim.log.levels.ERROR)
		return false
	end
