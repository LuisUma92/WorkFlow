...
function M.pick(opts)
  ...
  server.run_cli(args, config, function(ok, output)
    ...
    for _, entry in ipairs(decoded) do
      table.insert(items, {
        text = string.format("[%s] %-10s  %s  (%dlpw %dhpl)",
          entry.institution, entry.code, entry.name,
          entry.lectures_per_week, entry.hours_per_lecture),
        item = entry,
      })
    end
        ...
        if item then
          vim.notify(
            string.format("Selected: [%s] %s — %s (id=%d)",
              item.item.institution, item.item.code, item.item.name, item.item.id),
            vim.log.levels.INFO,
            { title = "workflow" }
          )
        end
