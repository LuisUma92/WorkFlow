-- lua/workflow/ui.lua
-- Forked from latexzettel/lua/latexzettel/ui.lua
-- Changes: title references updated to "workflow"
local M = {}
...
function M.prompt_form(title, fields, on_submit)
	...
	local lines = {}
	table.insert(lines, "Fill in each field and press <Enter> to submit. <Esc> to cancel.")
	table.insert(lines, "")
	...
	vim.api.nvim_buf_set_option(ibuf, "filetype", "workflow-form")

	local ilines = {}
	table.insert(ilines, "Enter values (one per line). Empty = default.")
	table.insert(ilines, "")
	for _, _ in ipairs(fields) do
		table.insert(ilines, "")
	end
