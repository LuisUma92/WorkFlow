...
- **`src/workflow/bibliography/`** — Bibliography foundation layer (ADR-0019, ADR-0020). `service.py` (`get_bib_entry_by_bibkey`, 0/1/2+ lookup contract), `dialect.py` (single biblatex↔bibtex alias layer: `to_biblatex`/`to_bibtex`, entry-type downgrade), `bibkey.py` (`calculate_bibkey`), `render.py` (`entry_to_biblatex`/`entry_to_bibtex`, shared by `prisma bib export` and accept-to-note), `inheritance.py` (crossref/xdata resolution behind `--resolve-xref`). `bibkey` stays nullable and non-unique by design (ADR-0019 §5 note, 2026-06-02).

- **`share/latex/sty/`** — 26 LaTeX style files (SetCommands, PartialCommands, SetExercises, VectorPGF, SetZettelkasten, etc.). Symlinked into projects from `~/.local/share/workflow/sty/`.

- **`share/latex/templates/`** — LaTeX templates for notes, exercises, lectures.

- **`share/latex/cls/`** — texnote.cls and preamble files (moved from latexzettel/defaults/template/).

- **`share/latex/pandoc/`** — Pandoc pipeline: filter.lua, template.tex, defaults.yaml, preprocess.py (moved from latexzettel/pandoc/).

- **`share/latex/ucimed/`** — UCIMED institutional bundle: evaluation styles (`eval-`, `format-`, `headers-`, `formulas-`, `colors-`, `comands-UCIMED.sty`; answer sheet + version logic, lab-guide `labguide` page style, tables captioned «Tabla»), Beamer theme (`beamertheme{UCIMED,UCIMEDpres}.sty` + color/font/inner/outer themes) and templates `UCIMED-Evaluacion-plantilla.tex` / `UCIMED-Presentacion-plantilla.tex`.
...
- Lectures CLI: `workflow lectures scan|split|link|build-eval`. `split --sync/--no-sync` (default `--sync`, Wave 0 D1): after splitting, runs `workflow.notes.sync.sync_note_files(paths, session)` over exactly the emitted files — registers notes + tags + `NoteConcept` + `main_topic` FK + `NoteEdge`; concept issues are stderr warnings (non-strict), exit 0. `sync_note_files` builds note_data from an explicit file list and calls the existing `_run_write_passes` with `skip_orphan_drop=True` — a subset sync can never drop Links of notes outside its list; `sync_vault` behavior unchanged (flag defaults False). Split output is written BEFORE sync — a DB failure surfaces after files exist, never corrupts split output.
...
- Vault CLI: `workflow vault info|validate|unify` (ITEP-0011, Implemented P0–P7). Migrates per-project `slipbox.db` notes into the global vault; idempotent via `.vault_pointer` marker. `.vault_pointer` is now (2026-08-08) a timestamp-only marker (`unified_at:`, no path) — it no longer carries a `vault_root:` path that could drift out of sync with `WORKFLOW_VAULT_ROOT`. `lectures split` defaults output to `<vault_root>/notes/permanent/` (override with `--output-dir`). Vault root resolved by `workflow.vault.paths.resolve_vault_root()` (env `WORKFLOW_VAULT_ROOT`, derives from `WORKFLOW_WORKSPACE_ROOT` when unset). Per-project note model: `ProjectNote` in `db/models/project_layer.py` (LocalBase, ITEP-0011 P5) — no CLI command yet.
- Concept CLI: `workflow concept list|show|add|tree|rm|rename|harvest` (ITEP-0012; harvest = Wave 0 D3). `concept harvest [--notes DIR|FILE ...] [--out PATH.yaml] [--json]` scans frontmatter `concepts:` lists, partitions against the Concept table via `resolve_concepts` (read-only — NEVER writes; `workflow import` stays the sole concept write path), and emits a skyfolding-delta YAML for unknown slugs: grouped by DisciplineArea trailing-AA match (`.order_by(code)`, bulk single query); suffix shared by >1 area → `AMBIGUOUS-PREFIX-<aa>` bucket listing all candidates (never a silent pick — 7 live areas end in MC); no match → `UNRECOGNIZED-PREFIX`. `domain: TODO-REVIEW` is a deliberately invalid VALUE so raw deltas fail `workflow import` until human-edited. Multi-bucket harvests write one importable file per bucket (`PATH-<bucket>.yaml`, `files: []` in `--json`); single bucket writes `PATH.yaml` verbatim. Zero unknowns → no file, exit 0. Manages the concept taxonomy (code slugs, parent hierarchy, content affiliation). `workflow concept add --code SLUG --label TEXT --content-id INT --domain DOMAIN [--parent CODE] [--description TEXT]`. Valid `--domain` values: `Información`, `Procedimiento Mental`, `Procedimiento Psicomotor`, `Metacognitivo` (from `_TAXONOMY_DOMAINS` in `workflow.db.models.knowledge`). `rm --force` reparents children to grandparent. Concept is now rooted at `Content` (not `MainTopic`); use `concept.main_topic` property for chain traversal. `resolve_concepts(codes, session, *, strict)` in `src/workflow/concept/service.py` is reused by the validator and MUST be reused by any future `notes link --concept` command. ADR: [ITEP-0012](docs/ADR/ITEP-0012-concept-orm.md). `notes link --concept CODE [--remove] [--strict]` materializes `NoteConcept` rows (P1, `dc79b59`). `notes link --main-topic CODE [--remove]` sets `Note.main_topic_id` FK (Phase 4D, v1.11.0). `notes sync` builds `NoteConcept` rows per-note from frontmatter `concepts:` list via Pass 5 `_sync_note_concepts` (`--strict-concepts` flag; P2, `2340d38`). Concept references are slug-only strict everywhere (`Concept.code`); labels are display-only (decision 2026-07-04, ITEP-0012 amendment).
- Notes authoring CLI (W1, ITEP-0015): `workflow notes capture --title TEXT [--type fleeting|literature|permanent (default fleeting)] [--tags a,b] [--concepts s1,s2] [--bibkey K] [--strict-concepts] [--json]` — creates `<zettel_id>-<slug>.md` FLAT in vault root (reuses the `new-id` generator), registers via `sync_note_files`; error-if-exists; strict fails before any write. `workflow notes promote REF [--json]` — FLIP-ONLY type change (fleeting/literature→permanent; frontmatter rewritten first via raw-dict dump preserving bibkey/origin, then sync flips DB; NEVER moves files — flat-layout amendment ITEP-0011 2026-07-05; same/unexpected type exit 1, traversal exit 2). `workflow notes search QUERY [--limit N] [--json]` — FTS5 standalone table `note_fts` (rowid pinned to Note.id, unicode61 remove_diacritics 2 so 'teoria' finds 'teoría'; phrase-quoted queries, no boolean operators; INNER JOIN filters ghost rows), maintained by the shared sync pass; `notes sync --rebuild-index` = full rebuild from .md files; migration 0017 also adds `note_alias`. ADR-0021 deviation documented in migration docstring: literal content='note' unworkable (Note has no body column) — standalone table preserves derived/rebuildable/single-authority properties. Note relation frontmatter (ITEP-0013, amended 2026-07-09) is 9 flat keys — `derived_from_{continuation,refines,branches,synthesis,rebuttal}` / `links_{supports,contradicts,expands,see_also}`, value = list of zettel_ids, empty keys omitted — because Obsidian Properties collapses a nested `relations:` mapping to a string and destroys the graph; `weight`/`note` are dropped from frontmatter (DB columns stay nullable, no migration). `parse_relations_frontmatter` is dual-format (flat wins); `workflow notes migrate-relations [--vault-root PATH] [--dry-run] [--json]` rewrites legacy nested notes on disk, per-note atomic, fails loudly on Obsidian-corrupted `relations:` strings (exit 1, restore from git). `workflow notes enums --json` additionally exposes `frontmatter_relation_keys` (9 derived entries) and `relation_key_prefixes` — read the key list from there, never hard-code it.
...
- Content bib-link CLI: `workflow content link-bib | bib-links | unlink-bib` (v1.13.0). Links `BibContent` rows (`bib_entry_id`, `content_id`) with locus columns (chapter, section, pages, exercise range). Bibkey resolution via `_resolve_bib_entry` (`content/bib_links.py`); the shared lookup lives in `workflow.bibliography.service.get_bib_entry_by_bibkey` (ADR-0020).
- Validation CLI: `workflow validate notes [--strict-main-topic] [--strict-concepts]` (ITEP-0009 + ITEP-0012, Implemented). Resolves frontmatter `main_topic` against `MainTopic`, enforces `discipline_area` consistency, and optionally validates `concepts:` slugs against the Concept table. `workflow validate exercises` also lints `\si{}`/`\SI{}{}` unit macros against SetUnits.sty declarations ∪ siunitx builtins (warn-only with difflib suggestion, never flips exit code; parser `workflow.latex.units.load_declared_units`, regex `\DeclareSIUnit\<name>{...}` control-sequence form only). `workflow validate config [PATHS]... [--json]` (2026-08-08) is a read-only config↔DB↔filesystem drift detector: compares each `config.yaml`'s `data.abs_parent_dir`/`abs_project_dir` against `workflow.db` and the real filesystem; finding kinds are `stale_path`, `path_mismatch`, `legacy_code_scheme`, `unregistered`, `db_mismatch`, `malformed`; exits 1 when any finding is present.
- Disciplines + maturation CLI: `workflow db disciplines list [--json]`, `workflow project propose-maturation [--json] [--area DDTTAA]` (ADR ITEP-0009). Bloom enums: `workflow item taxonomy --levels|--domains [--json]` (ADR ITEP-0006). `workflow project list [--json]` (2026-08-08) prints every registered `GeneralProject`/`LectureProject`. `workflow project adopt <PATH> [--dry-run] [--json]` (2026-08-08) registers a pre-existing on-disk repo as a `GeneralProject` by deriving area/year/initials/title from its `DDTTAA-YYPP-title` directory name; never creates the project tree or writes to disk; idempotent (`created:false` on repeat); malformed directory name exits 1, nonexistent path exits 2.
...
- `inittex` (2026-08-08) accepts `--area-code --title --year-init --project-initials`, all optional, threading through to `create_general(...)`'s existing kwargs — lets historical repos be adopted with their real creation year instead of always defaulting to the current year. `--year-init` is validated 0–99 (`MainTopic.code` is `String(10)` = `DDTTAA`+`YY`+`PP`). `--clone` combined with any of the four is a `click.UsageError`.
...
| ITEP-0008 | General project nomenclature (DDTTAA-YYPP-title) | Amended (2026-08-08) — naming/DB hierarchy implemented; physical two-layer nesting claim corrected, layout is flat per ITEP-0000 |
