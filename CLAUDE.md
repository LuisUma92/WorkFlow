...
- PRISMA CLI: `workflow prisma bib list|show`, `workflow prisma bib import <PATH>` (also `--stdin` to read biblatex from stdin), `workflow prisma keyword list`, `workflow prisma review list`, `workflow prisma checklist show`, `workflow prisma rationale add|list`, `workflow prisma tag add|list` (ADR PRISMA-0005)
