...
- PRISMA CLI: `workflow prisma bib list|show`, `workflow prisma bib import <PATH>` (also `--stdin` to read biblatex from stdin; `--recompute-bibkeys` to force calculated keys), `workflow prisma bib export [--dialect biblatex|bibtex]` (default biblatex; bibtex downgrades biblatex-only types + reverse-maps field names — ADR-0019), `workflow prisma bib recompute-keys [--dry-run] [--all] [--yes]` (fills missing calculated bibkeys by default; `--all` normalizes every key after confirmation; backs up the DB first — ADR-0019), `workflow prisma keyword list`, `workflow prisma review list`, `workflow prisma checklist show`, `workflow prisma rationale add|list`, `workflow prisma tag add|list` (ADR PRISMA-0005)
